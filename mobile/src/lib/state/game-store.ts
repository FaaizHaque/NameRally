...
export interface GameSession {
  ...
  status: 'lobby' | 'playing' | 'round_results' | 'final_results';
...
export const useGameStore = create<GameState>((set, get) => ({
  ...
  startGame: async () => {
    ...
    // Generate letter (no used letters for first round)
    const letter = generateRandomLetter([]);
    console.log('[GameStore] Starting game with letter:', letter);
    ...
    try {
      ...
      if (playersError) {
        console.error('[GameStore] Error resetting players:', playersError);
      }
      ...
      if (sessionError) {
        console.error('[GameStore] Error updating session:', sessionError);
      } else {
  ...
  endRound: async () => {
    ...
    if (isLocalSession) {
      ...
      if (statusError) {
        console.error('[GameStore] Error updating session status:', statusError);
      } else {
  ...
  nextRound: async () => {
    ...
    // Generate letter that hasn't been used yet
    const letter = generateRandomLetter(session.usedLetters);
    console.log('[GameStore] Starting next round with letter:', letter, 'Used letters:', session.usedLetters);
    ...
    try {
      ...
      if (playersError) {
        console.error('[GameStore] Error resetting players for next round:', playersError);
      }
      ...
      if (sessionError) {
        console.error('[GameStore] Error updating session for next round:', sessionError);
      } else {
        console.log('[GameStore] Session updated for next round');
      }
  ...
  endGameEarly: async () => {
    ...
    try {
      ...
      if (sessionError) {
        console.error('[GameStore] Error ending game early:', sessionError);
      } else {
