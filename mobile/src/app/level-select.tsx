...
export default function LevelSelectScreen() {
  ...
  const renderBandCard = (band: BandInfo, index: number) => {
    ...
    return (
      <Animated.View key={band.bandNumber} entering={FadeInUp.duration(400).delay(index * 50)}>
        <Pressable
          onPress={() => {
            if (isUnlocked) {
              Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
              setSelectedBand(selectedBand === band.bandNumber ? null : band.bandNumber);
            } else {
              Haptics.notificationAsync(Haptics.NotificationFeedbackType.Warning);
            }
          }}
          ...
          <View style={{
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <ChevronRight
                ...
                strokeWidth={2.5}
                style={{ transform: [{ rotate: selectedBand === band.bandNumber ? '90deg' : '0deg' }] }}
              />
            </View>
          </View>
        </Pressable>
        ...
        {/* Expanded level grid */}
        {selectedBand === band.bandNumber && isUnlocked && (
          <Animated.View
    ...
    <View style={{ flex: 1 }}>
      ...
      >
        <View style={{ paddingTop: insets.top, flex: 1 }}>
          ...
          {/* Play Next Button */}
          <Animated.View entering={FadeInUp.duration(500).delay(100)} style={{ paddingHorizontal: 16, marginBottom: 14 }}>
            <Pressable
          ...
          >
            {BANDS.map((band, index) => renderBandCard(band, index))}
          </ScrollView>
        </View>
      </LinearGradient>
    </View>
