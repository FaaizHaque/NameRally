...
const MODERN_CAT_COLORS: Record<string, { bg: string; border: string; icon: string }> = {
  names: { bg: '#1e1b4b', border: '#4F46E5', icon: '#a5b4fc' },
  ...
  sports_games: { bg: '#1a2240', border: '#6366f1', icon: '#a5b4fc' },
...
export default function FinalResultsScreen() {
  ...
  if (isTransitioning) {
    return (
      <LinearGradient colors={['#0d0d1a', '#1a1a2e']} style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
        <ActivityIndicator size="large" color="#6366f1" />
        <Text style={{ color: '#a5b4fc', marginTop: 12, fontSize: 14 }}>Loading next level...</Text>
      </LinearGradient>
    );
  }
  ...
  const handlePlayAgain = async () => {
    ...
    await leaveGame();
    if (isLevelMode) router.replace('/level-select');
  ...
  if (isLevelMode && currentLevel) {
    ...
    return (
      <LinearGradient colors={['#0d0d1a', '#1a1a2e', '#12122a']} style={{ flex: 1 }}>
        <Pressable style={{ flex: 1 }} onPress={revealStage < 3 ? skipToEnd : undefined}>
          <ScrollView
            style={{ flex: 1 }}
          ...
          >
            ...
            >
              <View style={{ backgroundColor: '#1e1b4b', paddingHorizontal: 14, paddingVertical: 4, borderRadius: 12, marginBottom: 14, borderWidth: 1.5, borderColor: '#4F46E5' }}>
                <Text style={{ color: '#a5b4fc', fontSize: 11, fontWeight: '700', letterSpacing: 2 }}>LEVEL {currentLevel.level}</Text>
              </View>

              <Animated.View style={trophyAnimStyle}>
                <View style={{
                  width: 110, height: 110, borderRadius: 55,
                  backgroundColor: levelPassed ? '#1e1b4b' : '#2a0a0a',
                  alignItems: 'center', justifyContent: 'center',
                  borderWidth: 3, borderColor: levelPassed ? '#6366f1' : '#ef4444',
                  shadowColor: levelPassed ? '#4F46E5' : '#ef4444',
                  shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.7, shadowRadius: 24,
                }}>
                  {levelPassed
                    ? <CheckCircle size={58} color="#6366f1" />
              ...
              {revealStage < 1 && (
                <Text style={{ color: 'rgba(165,180,252,0.35)', fontSize: 11, marginTop: 12, fontWeight: '600', letterSpacing: 1 }}>
                  tap to skip
                </Text>
              )}
            </Animated.View>
            ...
            {revealStage >= 1 && (
              <Animated.View entering={FadeInUp.duration(500).springify().damping(16)} style={{ alignItems: 'center', paddingVertical: 4 }}>
                ...
                <Animated.View entering={FadeInDown.duration(400).delay(100)}>
                  ...
                  <Text style={{ color: '#6366f1', fontSize: 14, textAlign: 'center', marginTop: -4 }}>/ {currentLevel.maxPossibleScore} points</Text>
                </Animated.View>
                ...
                <Animated.View entering={FadeIn.duration(350).delay(250)} style={{
                  ...
                  backgroundColor: levelPassed ? '#1e1b4b' : '#2a0a0a',
                  borderWidth: 1.5, borderColor: levelPassed ? '#6366f1' : '#ef4444',
                }}>
            ...
            {revealStage >= 2 && latestRound && (
              <Animated.View entering={FadeInUp.duration(450).springify().damping(16)}>
                <Text style={{ color: '#818CF8', fontSize: 11, textAlign: 'center', marginBottom: 8, fontWeight: '700', letterSpacing: 1.5 }}>YOUR ANSWERS</Text>
                <View style={{ backgroundColor: '#111827', borderRadius: 14, padding: 10, borderWidth: 1.5, borderColor: '#1f2937', gap: 6 }}>
                  {session?.settings.selectedCategories.map((category, index) => {
                    ...
                    const mc = MODERN_CAT_COLORS[category] || { bg: '#1a1a2e', border: '#4F46E5', icon: '#a5b4fc' };
                      ...
                      >
                        ...
                        <View style={{ flex: 1 }}>
                          ...
                          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5 }}>
                            ...
                            {hasBonus && (
                              <View style={{ backgroundColor: '#2a2010', paddingHorizontal: 4, paddingVertical: 1, borderRadius: 5, flexDirection: 'row', alignItems: 'center', gap: 2, borderWidth: 1, borderColor: '#f59e0b' }}>
                                <Zap size={8} color="#fcd34d" strokeWidth={2.5} />
            ...
            {revealStage >= 3 && (
              <Animated.View entering={FadeInUp.duration(450).springify().damping(16)} style={{ gap: 12 }}>
                ...
                <View style={{
                  ...
                  {[
                    ...
                    <React.Fragment key={item.label}>
                      {i > 0 && <View style={{ width: 1, backgroundColor: '#1f2937' }} />}
                      <View style={{ alignItems: 'center', flex: 1 }}>
                        <Text style={{ color: '#6366f1', fontSize: 9, marginBottom: 4, fontWeight: '700', letterSpacing: 1 }}>{item.label}</Text>
                ...
                <LinearGradient
                  ...
                  style={{ borderRadius: 14, padding: 16, borderWidth: 1.5, borderColor: '#4F46E5' }}
                >
                  <Text style={{ color: '#818CF8', fontSize: 11, textAlign: 'center', marginBottom: 12, fontWeight: '700', letterSpacing: 1 }}>TOTAL PROGRESS</Text>
                  <View style={{ flexDirection: 'row', justifyContent: 'space-around' }}>
                    <View style={{ alignItems: 'center' }}>
                      ...
                      <Text style={{ color: '#6366f1', fontSize: 10, marginTop: 4 }}>Total Points</Text>
                    </View>
                    <View style={{ width: 1, backgroundColor: '#4F46E530' }} />
                    <View style={{ alignItems: 'center' }}>
                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                      ...
                      <Text style={{ color: '#6366f1', fontSize: 10, marginTop: 4 }}>Total Stars</Text>
                    </View>
                  </View>
                </LinearGradient>
                ...
                <View style={{ gap: 10 }}>
                  {levelPassed ? (
                    <Pressable onPress={handlePlayNextLevel} disabled={isLoadingNextLevel} style={({ pressed }) => ({ transform: [{ scale: pressed ? 0.97 : 1 }] })}>
                      <LinearGradient
                        ...
                        style={{
                          ...
                          shadowColor: '#4F46E5', shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.5, shadowRadius: 16, elevation: 10,
                        }}
                      >
                        {isLoadingNextLevel ? <ActivityIndicator color="#ffffff" /> : (
                          <>
                            <Play size={22} color="#fff" fill="#fff" strokeWidth={2} />
                            <Text style={{ color: '#fff', fontWeight: '900', fontSize: 18, letterSpacing: 0.5 }}>
                              {currentLevel.level < 500 ? `Play Level ${currentLevel.level + 1}` : 'All Levels Complete!'}
                            </Text>
                    ...
                    <Pressable onPress={handleRetryLevel} disabled={isLoadingNextLevel} style={({ pressed }) => ({ transform: [{ scale: pressed ? 0.97 : 1 }] })}>
                      <View style={{
                        ...
                        flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10,
                        shadowColor: '#ef4444', shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.4, shadowRadius: 12, elevation: 8,
                      }}>
                        {isLoadingNextLevel ? <ActivityIndicator color="#ef4444" /> : (
                          <>
                            <RotateCcw size={22} color="#ef4444" strokeWidth={2.5} />
                            <Text style={{ color: '#fca5a5', fontWeight: '900', fontSize: 18 }}>Retry Level {currentLevel.level}</Text>
                          </>
                        )}
                      </View>
                    </Pressable>
                  ...
                  <View style={{ flexDirection: 'row', gap: 10 }}>
                    <Pressable onPress={handleGoHome} style={({ pressed }) => ({ flex: 1, transform: [{ scale: pressed ? 0.97 : 1 }] })}>
                      <View style={{ backgroundColor: '#111827', borderRadius: 14, paddingVertical: 16, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, borderWidth: 1.5, borderColor: '#1f2937' }}>
                        <Home size={18} color="#6366f1" strokeWidth={2.5} />
                        <Text style={{ color: '#a5b4fc', fontWeight: '700', fontSize: 15 }}>Home</Text>
                      </View>
                    </Pressable>
                    <Pressable onPress={handlePlayAgain} style={({ pressed }) => ({ flex: 1, transform: [{ scale: pressed ? 0.97 : 1 }] })}>
                      <View style={{ backgroundColor: '#111827', borderRadius: 14, paddingVertical: 16, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, borderWidth: 1.5, borderColor: '#1f2937' }}>
                        <RotateCcw size={18} color="#6366f1" strokeWidth={2.5} />
                        <Text style={{ color: '#a5b4fc', fontWeight: '700', fontSize: 15 }}>All Levels</Text>
                      </View>
                    </Pressable>
                  </View>
                </View>
