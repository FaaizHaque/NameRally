...
type TabType = 'basics' | 'single' | 'daily' | 'multiplayer';
    ...
    <View className="flex-1">
      ...
      >
        <View style={{ paddingTop: insets.top }} className="flex-1">
          ...
          >
            <View className="flex-row bg-white/10 rounded-2xl p-1.5 flex-1">
              {tabs.map((tab) => (
                <Pressable
                  key={tab.id}
                ...
                >
                  <View
                    className={`flex-row items-center justify-center gap-1.5 py-2.5 rounded-xl ${
                      activeTab === tab.id ? 'bg-[#3BA99C]' : ''
                    }`}
                  >
                    {tab.icon}
                    <Text
                      className={`text-sm font-semibold ${
                        activeTab === tab.id ? 'text-[#1a1a2e]' : 'text-white/70'
                      }`}
                    >
                      {tab.label}
                    </Text>
